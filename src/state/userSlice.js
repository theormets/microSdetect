...
export const register =
  ...
  () => {
    const { firstName, lastName, email, password, organization, description} = values;
    instance
...
export const getAllStudents = async (page, rowsPerPage) => {
    try {
        console.log("getting all");
        const response = await privateInstance.post("/api/v1/admin/students", {
            page,
            rowsPerPage,
        });
...
export const updateStudentStatus = async (userId, status) => {
    try {
        const response = await privateInstance.patch("/api/v1/admin/studentstatus", {
            userId,
            status,
        });
